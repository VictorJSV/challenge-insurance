...
import IconDown from '@src/assets/svgs/icon-down-arrow.svg?react';
...
const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ options, isError, label, disabled, alignToRight, variant = 'default', ...props }, ref) => {
    ...
    return (
      <div className="c-select" ref={wrapperRef}>
        {label && (
          <label htmlFor={props.id || props.name} className="c-select__label">
            {label}
          </label>
        )}
        <select
        ...
        <button
          ...
          aria-controls={`${props.id || props.name}Listbox`}
        ...
        {isOpen && (
          <ul className="c-select__listbox" id={`${props.id || props.name}Listbox`}>
            {options.map((opt) => (
              <li
                key={opt.value}
                className={classes(
                ...
                onClick={() => handleClickOption(String(opt.value))}
              >
                {opt.label}
                {value === opt.value && (
